...
public class Branching {
	 ...
	 * @param A_values Current valufes for each of the unique primitives being evaluated
	...
	public static ReturnObject isSatisfiable(String[][] L, Boolean[][] L_values, String[] A, Boolean[] A_values, String next_p) {		
		...
		for (int i=0; i<L.length; i++) {
			...
			boolean x = false; 
			 for (int j =0; j <L[i].length; j++){
				if (L_values[i][j]!= null && L_values[i][j] == true ) //[your code here] 
				{
					x = true;
				}
	    ...
	    if(DEBUG) System.out.println("L_new:" + Arrays.deepToString(L_new));
	    
	    // 3. If there are no clauses left (i.e. they have all been satisfied and thus discarded), then return TRUE.
	    if (L_new.length==0)// [your code here] 
	    {
	    ...
	    if (a!=-1) {
			A_values[a] = true;
			String [][] L_clone = L_new.clone();
			Boolean [][] L_values_clone = L_values_new.clone();
		    ...
		    ReturnObject output = isSatisfiable(L_clone,L_values_clone, A, A_values, A[a]); // [your code here]
		    // L_values_clone= L_values_new  ;
		    ...
		    ReturnObject output2 = isSatisfiable(L_clone,L_values_clone, A, A_values, A[a]); // [your code here]
